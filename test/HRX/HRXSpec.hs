...
spec = parallel $ do
  describe "parse" $ do
    ...
    it "parses contents without a newline" $
      testParse pArchive "<===> file\ncontents" `parseSatisfies` ((== Just "contents") . entryContent . entryData . snd . head . archiveEntries)
    ...
    describe "with a single file" $ do
      ...
      it "parses the filename" $ t testParse `parseSatisfies` ((== Path "file") . fst . head . archiveEntries)
      it "parses the contents" $ t testParse `parseSatisfies` ((== Just "contents\n") . entryContent . entryData . snd . head . archiveEntries)

    describe "parses contents with boundary-like sequences" $ do
      let t p = p pArchive (T.unlines ["<===> file", "<==>", "inline <===>", "<====>"])

      it "parses the contents" $
        t testParse `parseSatisfies` ((== Just (T.unlines ["<==>", "inline <===>", "<====>"])) . entryContent . entryData . snd . head . archiveEntries)
    ...
    describe "with multiple files" $ do
      ...
      it "parses the first filename" $ oT testParse `parseSatisfies` ((== Path "file 1") . fst . head . archiveEntries)
      it "parses the first contents" $ oT testParse `parseSatisfies` ((== Just "contents 1\n\n") . entryContent . entryData . snd . head . archiveEntries)
      it "parses the second filename" $ oT testParse `parseSatisfies` ((== Path "file 2") . fst . last . archiveEntries)
      it "parses the second contents" $ oT testParse `parseSatisfies` ((== Just "contents 2\n") . entryContent . entryData . snd . last . archiveEntries)
      ...
      describe "with a comment" $ do
        ...
        it "parses the first filename" $ t testParse `parseSatisfies` ((== Path "file 1") . fst . head . archiveEntries)
        it "parses the first contents" $ t testParse `parseSatisfies` ((== Just "contents 1\n") . entryContent . entryData . snd . head . archiveEntries)
        it "parses the second filename" $ t testParse `parseSatisfies` ((== Path "file 2") . fst . last . archiveEntries)
        it "parses the second contents" $ t testParse `parseSatisfies` ((== Just "contents 2\n") . entryContent . entryData . snd . last . archiveEntries)
        it "parses the comment" $ t testParse `parseSatisfies` ((== Just "comment\n") . entryComment . snd . last . archiveEntries)
    ...
    describe "with a single directory" $ do
      ...
      it "parses one entry" $ t testParse `parseSatisfies` ((== 1) . length . archiveEntries)
      it "parses a diretory and the filename" $ t testParse `parseSatisfies` ((== Path "dir/") . fst . head . archiveEntries)
